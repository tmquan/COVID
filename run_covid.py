...
class CustomBinaryStatistics(object):
    ...
    def __init__(self, threshold=0.5, types=6):
        ...
        self.types = types
        self.average_type = 'binary' if self.types == 1 else 'weighted'
    ...
    def precision(self):
        ...
        return sklearn.metrics.precision_score(np_label, np_estim, average=self.average_type)
    ...
    def f1_score(self):
        np_label = np.array(self.total_label).astype(np.float32).reshape(-1, self.types)
        np_estim = np.array(self.total_estim).astype(np.float32).reshape(-1, self.types)
        return sklearn.metrics.f1_score(np_label, np_estim, average=self.average_type)

    @property
    def f2_score(self):
        np_label = np.array(self.total_label).astype(np.float32).reshape(-1, self.types)
        np_estim = np.array(self.total_estim).astype(np.float32).reshape(-1, self.types)
        return sklearn.metrics.fbeta_score(np_label, np_estim, beta=2, average=self.average_type)
